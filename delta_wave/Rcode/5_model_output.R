...
for (i in 1:length(mod.name)){
  load(paste0("output/", region_subset, "/", mod.name[i],".RData"))
...
fwrite(table1, file = paste0("figs/", region_subset, "/table_S01.csv"), quote = FALSE, 
          row.names = FALSE) 

# load baseline and selected best fit model
load(paste0("output/", region_subset, "/basemodel.RData"))
basemodel <- model
summary(basemodel)
...
load(paste0("output/", region_subset, "/", best.model, ".RData"))
summary(model)
...
write.csv(daily_effects,file = paste0("figs/", region_subset, "/table_S02_week_effectsi.csv"),row.names = F)
...
write.csv(space, file = paste0("figs/", region_subset, "/table_S03_spatial random effects per wave.csv"),row.names = F)
...
ggsave(paste0("figs/", region_subset, "/fig_S05_week_spatial_effect.pdf"), height = 20, width = 30, units = "cm")
...
mn <-min(MAE$diff)
mx <-max(MAE$diff)
write.csv(MAE,file = paste0("figs/", region_subset, "/table_S04_MAEi.csv"),row.names = F)
...
ggsave(value_map, filename = paste0("figs/", region_subset, "/fig_S06_value_map.pdf"),  width=6, height=7)
...
# Appendix Table S2
write.csv(tableS2, file = paste0("figs/", region_subset, "/table_S05_Added_value_region.csv"), quote = FALSE, 
          row.names = FALSE) 
...
pdf(paste0("figs/", region_subset, "/fig_S07_maps_fitted_Rt_by_week.pdf"), height = 8, width = 12)
Rt_pred_week
dev.off()
...
pdf(paste0("figs/", region_subset, "/fig_S08_Rt_obs_vs_fitted_state.pdf"), height = 12, width = 12)
Rt_ts_prov
dev.off()
...
for (i in 1:nweek){ # no of weeks
    load(paste0("output/", region_subset, "/preds/preds_week", i, ".RData"))
...
pdf(paste0("figs/", region_subset, "/fig_S07_maps_Rt_by_week.pdf"), height = 8, width = 12)
Rt_week
dev.off()
...
pdf(paste0("figs/", region_subset, "/fig_S07_maps_pred_Rt_by_week.pdf"), height = 8, width = 12)
Rt_pred_week
dev.off()
...
pdf(paste0("figs/", region_subset, "/fig_S08_Rt_obs_pred_state.pdf"), height = 12, width = 12)
Rt_ts_prov
dev.off()
