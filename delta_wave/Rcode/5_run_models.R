...
save(model, file = paste0("output/", region_subset, "/basemodel.RData"))
...
models <- lapply(1:length(formulas),
              function(i) {
                ...
                save(model, file = paste0("output/", region_subset, "/", lab[i],".RData"))})
...
for(i in 1:length(formulas)){
  load(paste0("output/", region_subset, "/",lab[i],".RData"))
...
fwrite(table1, file = paste0("output/", region_subset, "/best_model_selection1.csv"), quote = FALSE,
       row.names = FALSE)
...
(lambda <- 3*sdr/tan(pi*0.99/2))# 0.03522783
...
precision.prior <- list(prec = list(prior="pc.prec", param = c(3*sdr,0.01)))
precision.prior <- list(prec = list(prior="pc.prec", param = c(2*sdr,0.01)))
precision.prior <- list(prec = list(prior="pc.prec", param = c(sdr,0.01)))

precision.prior <- list(prec = list(prior = halfcauchy))#6431.20

precision.prior <- list(prec = list(prior = "pc.prec", param = c(0.5, 0.01)))#dic = 6429.753
precision.prior <- list(prec = list(prior = "pc.prec", param = c(0.5, 0.001)))#dic = 6431.52
precision.prior <- list(prec = list(prior = "pc.prec", param = c(0.5, 0.1)))#dic = 6429.52
precision.prior <- list(prec = list(prior = "pc.prec", param = c(1, 0.001))) #dic = 6431.124
precision.prior <- list(prec = list(prior = "pc.prec", param = c(0.1, 0.01))) 
precision.prior <- list(prec = list(prior = "pc.prec", param = c(0.1, 0.001))) 
