...
### load lag data for model without DLNM ----
...
## wave1 lag 1-week data
# data_all <- fread("data/data_weekly_all_lag1week_merged_wave1.csv", stringsAsFactors = F)

## wave1 lag 2-week data
# data_all <- fread("data/data_weekly_all_lag2week_merged_wave1.csv", stringsAsFactors = F)

## wave2 lag 1-week data
# data_all <- fread("data/data_weekly_all_lag1week_merged_wave2.csv", stringsAsFactors = F)

## wave2 lag 2-week data
data_all <- fread("data/data_weekly_all_lag2week_merged_wave2.csv", stringsAsFactors = F)
...
fit = glm(R ~ 1 +  cases_accu_rate + mean_intra + d2m + t2m + tp + uv + Stringency + 
          ...
          data = data_all, family = Gamma(link = "log")) #AIC: 11430
summary(fit)
...
fit = glm(ln_R ~ cases_accu_rate + mean_intra + t2m + uv + Stringency + 
            holiday + offset(log(R0)), 
          data = data_all, family = 'gaussian') 
fit$aic
