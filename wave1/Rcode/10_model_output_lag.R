...
head(data)
# df <- data[ -c(27:34)]
# data[,-which(names(data) == "region.x", "NAME_1.x")]
# data

data[ , -which(names(data) %in% c("region.x","NAME_1.x"))]
data
...
for (i in 1:length(mod.name)){
  load(paste0("output_lag/whole country/", mod.name[i],".RData"))
...
fwrite(table1, file = "figs_lag/whole country/table_S01.csv", quote = FALSE, 
          row.names = FALSE) 

# load baseline and selected best fit model
load("output_lag/whole country/basemodel.RData")
basemodel <- model
summary(basemodel)
...
load(paste0("output_lag/whole country/", best.model, ".RData"))
summary(model)
...
write.csv(daily_effects,file = "figs_lag/whole country/table_S02_week_effectsi.csv",row.names = F)
...
write.csv(space, file = "figs_lag/whole country/table_S03_spatial random effects per wave.csv",row.names = F)
...
ggsave("figs_lag/whole country/fig_S05_week_spatial_effect.pdf", height = 20, width = 30, units = "cm")
...
mn <-min(MAE$diff)
mx <-max(MAE$diff)
write.csv(MAE,file = "figs_lag/whole country/table_S04_MAEi.csv",row.names = F)
...
ggsave(value_map, filename = "figs_lag/whole country/fig_S06_value_map.pdf",  width=6, height=7)
...
# Appendix Table S2
write.csv(tableS2, file = "figs_lag/whole country/table_S05_Added_value_region.csv", quote = FALSE, 
          row.names = FALSE) 
...
pdf("figs_lag/whole country/fig_S07_maps_fitted_Rt_by_week.pdf", height = 8, width = 12)
Rt_pred_week
dev.off()
...
pdf("figs_lag/whole country/fig_S08_Rt_obs_vs_fitted_state.pdf", height = 12, width = 12)
Rt_ts_prov
dev.off()
...
for (i in 1:nweek){ # no of weeks
    load(paste0("output_lag/whole country/preds/preds_week", i, ".RData"))
...
pdf("figs_lag/whole country/fig_S07_maps_Rt_by_week.pdf", height = 8, width = 12)
Rt_week
dev.off()
...
pdf("figs_lag/whole country/fig_S07_maps_pred_Rt_mean_by_week.pdf", height = 8, width = 12)
Rt_pred_week
dev.off()
...
pdf("figs_lag/whole country/fig_S07_maps_pred_Rt_sd_by_week.pdf", height = 8, width = 12)
Rt_pred_week
dev.off()
...
pdf("figs_lag/whole country/fig_S07_maps_pred_Rt_lci_by_week.pdf", height = 8, width = 12)
Rt_pred_week
dev.off()
...
pdf("figs_lag/whole country/fig_S07_maps_pred_Rt_uci_by_week.pdf", height = 8, width = 12)
Rt_pred_week
dev.off()
...
pdf("figs_lag/whole country/fig_S08_Rt_obs_vs_pred_state_capital.pdf", height = 12, width = 12)
Rt_ts_prov
dev.off()
