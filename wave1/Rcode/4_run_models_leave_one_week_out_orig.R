...
# formula <- Y ~ f(W1, model = "rw1", hyper = precision.prior) + f(S1, model = "bym2", 
#   scale.model = TRUE, graph = g, hyper = precision.prior) + 
#   Vp + basis_GLH + basis_TEMP + basis_PREC + Vh
...
for(wweek in unique(data$week_index)){
  ...
  save(preds, file = paste0("output/whole country/preds/preds_week",0 + wweek, ".RData"))
